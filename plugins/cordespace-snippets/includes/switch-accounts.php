<?php
/**
 * Switch entre comptes liés (ex-WPCode snippet 1224).
 *
 * Permet à une même personne d'avoir deux comptes WP (un client·e Amelia,
 * un prof Amelia) et de basculer entre eux en un clic via User Switching.
 *
...
/**
 * Cordespace — Liaison entre 2 comptes WP d'une même personne
 * (typiquement : un compte "client·e" Amelia + un compte "prof" Amelia).
 *
 ...
 * - Fournit un shortcode [cordespace_switch_button] avec redirect intelligent.
 * - Vide les cookies Amelia à chaque switch (sinon l'ancien user "colle").
 *
 * Dépend de : User Switching plugin (john-blackbourn).
 */
...
//    Avec la page unifiée, on redirige toujours vers /mon-espace/
//    qui s'adapte automatiquement au rôle de l'utilisateur·trice.
...
function cordespace_user_switching_redirect_filter( $redirect_to, $context = '', $current_user = null, $new_user = null ) {
	if ( ! empty( $_REQUEST['redirect_to'] ) ) {
		...
		$home   = home_url();
		// Accepte les URLs du même domaine ou les chemins vers /mon-espace/
