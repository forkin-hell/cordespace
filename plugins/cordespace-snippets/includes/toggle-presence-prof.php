<?php
/**
 * Toggle présence prof + shortcode [cordespace_today_students] (ex-WPCode 1249).
 *
 * - Crée la table wphu_cordespace_checkins (idempotent via dbDelta)
 ...
 *
 * Dépendances : Amelia, helper cordespace_user_is_amelia_provider() défini
 * dans mon-espace-shortcode.php (ordre de chargement alphabétique garanti).
 */

defined( 'ABSPATH' ) || exit;

/**
 * Cordespace — Toggle présence prof (Phase 2 — v2 avec switch iOS)
 *
 * - Liste les élèves inscrit·es aux cours du prof dans les 24h
 * - Toggle "Présent·e" (violet) / "À venir" (gris) en switch iOS
 * - Stockage : table custom wp_cordespace_checkins (ne touche pas Amelia)
